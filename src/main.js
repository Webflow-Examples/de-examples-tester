...
const App = () => {
  ...
  const { functionCode, parameterNames, functionParameters, setFunctionParameters } = useFunctionCode(selectedFunctionName, selectedExampleCategory);

  // Creating options for the dropdowns
  const exampleCategories = Object.keys(examples).map(key => ({ value: key, label: key }));
  ...
  return (
    <div>
      ...
      {parameterNames.length > 0 && (
        <button onClick={handleFunctionExecutionWithParameters}>Run Function</button>
      )}
